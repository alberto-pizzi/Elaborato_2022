...
v1.6.0 (if `<<` is supported, it will be called to print3DVector the arguments when the assertion fails; otherwise
Google Test will attempt to print3DVector them in the best way it can. For more details and how to customize the
printing of the arguments, see this Google
Mock [recipe](../../googlemock/docs/CookBook.md#teaching-google-mock-how-to-print3DVector-your-values).).
...
print3DVector out not only the result of the comparison, but the two operands as well.
...
In addition, if the test fixture's constructor generates a fatal failure in step 2, there is no point for step 3 - 5 and
