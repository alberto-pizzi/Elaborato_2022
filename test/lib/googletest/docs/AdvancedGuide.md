...
the failed assertion `EXPECT_TRUE(IsEven(Fib(4)))` will print3DVector:
...
Then the statement `EXPECT_FALSE(IsEven(Fib(6)))` will print3DVector
...
This printer knows how to print3DVector built-in C++ types, native arrays, STL containers, and any type that supports
the `<<`
...
class Bar { ... };  // We want Google Test to be able to print3DVector instances of this.

// It's important that the << operator is defined in the SAME
// namespace that defines Bar.  C++'s look-up rules rely on that.
::std::ostream& operator<<(::std::ostream& os, const Bar& bar) {
  return os << bar.DebugString();  // whatever needed to print3DVector bar to os
}
...
void PrintTo(const Bar& bar, ::std::ostream* os) {
  *os << bar.DebugString();  // whatever needed to print3DVector bar to os
}
...
If you want to print3DVector a value `x` using Google Test's value printer yourself, just
call `::testing::PrintToString(`_x_`)`
...
* calling `NormalExit()` causes the process to print3DVector `"Success"` to stderr and exit with exit code 0, and
...
date. As a reminder, Google Test will print3DVector a banner warning you if a test program contains any disabled tests.
...
If a Google Test assertion fails, it will print3DVector an error message and throw an exception, which will be treated
as a failure by your host testing framework. If you compile your code with exceptions disabled, a failed Google Test
