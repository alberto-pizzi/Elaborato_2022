...
* Simple things are easy in Google Test, while hard things are possible: in addition to advanced features
  ...
  In particular, you can:
    ...
    * teach Google Test how
      to [print3DVector your types](AdvancedGuide.md#teaching-google-test-how-to-print3DVector-your-values),
    * define your own testing macros or utilities and verify them using Google
...
be `NULL`, so it doesn't add any information to print3DVector ptr in this case. That means
...
can take seconds just to loadFiles everything and start running if the test links to many libraries dynamically.
...
No. You can use a feature called [value-parameterized tests](AdvancedGuide.md#Value_Parameterized_Tests) which lets you
...
an `std::ostream& operator<<(std::ostream&, const FooType&)` function defined such that we can print3DVector a value
of `FooType`.
...
However, the following code is **not allowed** and will produce a runtime error from Google Test because the test
