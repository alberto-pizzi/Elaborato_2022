...
HandgunBullet::HandgunBullet(const sf::Texture &tex, float speed, const sf::Vector2f &barrelHole,
    ...
    if (weaponScale.x > 0)
        bulletOrigin = {static_cast<float>(weaponPos.x + (barrelHole.x) * std::cos((M_PI * (-degrees)) / 180) +
                                           tex.getSize().y * std::sin((M_PI * (-degrees)) / 180)),
                        static_cast<float>(weaponPos.y + (barrelHole.x) * std::sin((M_PI * (degrees)) / 180) +
                                           tex.getSize().y * std::cos((M_PI * (-degrees)) / 180))};
    else
        bulletOrigin = {static_cast<float>(weaponPos.x - (barrelHole.x) * std::cos((M_PI * (degrees)) / 180) -
                                           tex.getSize().y * std::sin((M_PI * (degrees)) / 180)),
                        static_cast<float>(weaponPos.y + (barrelHole.x) * std::sin((M_PI * (-degrees)) / 180) +
